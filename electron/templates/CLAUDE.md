...
## Build Scripts (REQUIRED in package.json)
The build MUST copy static files (HTML, CSS) to dist/:
```json
"scripts": {
  "postinstall": "electron-rebuild 2>/dev/null; codesign --force --deep --sign - node_modules/electron/dist/Electron.app 2>/dev/null || true; find node_modules -name '*.node' -exec codesign --force --sign - {} \\; 2>/dev/null || true",
  "build": "tsc && npm run build:renderer && npm run copy:static",
  "build:renderer": "esbuild src/renderer/renderer.ts --bundle --outfile=dist/renderer/renderer.js --platform=browser",
  "copy:static": "cp src/renderer/*.html src/renderer/*.css dist/renderer/ 2>/dev/null || true"
}
```
Without `copy:static`, Electron will crash because `dist/renderer/index.html` is missing!

## Commands
```bash
npm run build          # TypeScript + esbuild bundle + copy static files
...
- Example: `features/gaeb-import.feature` → `e2e/gaeb-import.spec.ts`
- Example: `features/project-management.feature` → `e2e/project-management.spec.ts`
